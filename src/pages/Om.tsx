    ...
    <div className="bg-ek-50">
      ...
      <section className="bg-grafit-900 pt-36 pb-20">
        <div className="mx-auto max-w-content px-6">
          <p className="overlinje mb-4 text-massing-ljus">Om oss</p>
      ...
      {/* Team */}
      <section className="bg-grafit-900 text-ek-50">
        <div className="mx-auto max-w-content px-6 py-20">
          <p className="overlinje mb-4 text-massing-ljus">Människorna</p>
          <h2 className="mb-12 font-serif text-4xl text-ek-50">
            Vilka du pratar med
          </h2>
          <div className="grid gap-8 sm:grid-cols-2">
            {kontakter.map((k) => (
              <div
                key={k.epost}
                className="flex items-center gap-5 rounded-sm bg-grafit-800 p-7"
              >
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-massing/20 font-serif text-2xl text-massing-ljus">
                  {k.namn
                    .split(' ')
                    .map((n) => n[0])
                    .join('')}
                ...
                <div>
                  <h3 className="font-serif text-xl text-ek-50">{k.namn}</h3>
                  <p className="text-sm uppercase tracking-wide text-massing-ljus">
                    {k.roll}
                  </p>
                  <p className="mt-2 text-sm text-ek-100/75">{k.telefon}</p>
                  <a
                    href={`mailto:${k.epost}`}
                    className="text-sm text-ek-100/75 transition-colors hover:text-massing-ljus"
                  >
                    {k.epost}
                  </a>
                </div>
