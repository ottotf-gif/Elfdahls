...
// Riktiga bilder i public/ (en per kort)
...
export default function Hem({ navigera, oppnaTjanst }: HemProps) {
  return (
    <div>
      {/* ── HERO ─────────────────────────────────────────────── */}
      {/* Mörkare bara här, för att vit/guld-loggan och rubriken ska synas mot bilden */}
      <section
        id="hem"
      ...
      >
        ...
        />
        {/* Lättare overlay än tidigare – mer luft, mindre dysterhet */}
        <div className="absolute inset-0 bg-gradient-to-r from-grafit-950/85 via-grafit-900/60 to-grafit-900/20" />
        ...
        <div className="relative mx-auto w-full max-w-content px-6 py-32">
          <div className="max-w-2xl animate-fadeUp">
            ...
            <p className="mt-7 max-w-xl text-lg leading-relaxed text-ek-100/90">
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              ...
              <button
                ...
                className="knapp-sekundar border-ek-100/50 text-ek-50 hover:border-ek-50 hover:bg-ek-50 hover:text-grafit-900"
              >
                Se våra projekt
              </button>
            </div>
        ...
        <div className="absolute bottom-8 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-ek-100/60 md:flex">
          <span className="text-[10px] uppercase tracking-widest2">Scrolla</span>
          <span className="h-10 w-px bg-ek-100/40" />
        </div>
      </section>
      ...
      {/* ── TJÄNSTER (ljus sektion, bild över, titel under – klickbara) ── */}
      <section id="tjanster" className="border-y border-ek-200 bg-ek-100">
        <div className="mx-auto max-w-content px-6 py-24">
          <div className="mb-14 text-center">
            <p className="overlinje mb-4 text-massing-mork">Vår verksamhet</p>
            <h2 className="font-serif text-4xl text-grafit-900 sm:text-5xl">
              Det vi gör bäst
            </h2>
            <p className="mx-auto mt-4 max-w-md text-grafit-600">
          ...
          <div className="grid gap-x-10 gap-y-14 sm:grid-cols-2">
            {tjanster.map((t) => (
              ...
              >
                <div className="relative aspect-[5/3] overflow-hidden rounded-sm bg-ek-200 shadow-mjuk">
                  <img
                    src={kortbild[t.id]}
                    alt={t.titel}
                    loading="lazy"
                ...
                <h3 className="mt-6 font-serif text-2xl text-grafit-900 transition-colors group-hover:text-massing-mork sm:text-3xl">
                  {t.titel}
                </h3>
                <span className="signatur-linje my-3" />
                <p className="max-w-md text-sm leading-relaxed text-grafit-600">
                  {t.kort}
                </p>
                <span className="mt-4 inline-block text-sm font-medium uppercase tracking-widest text-massing-mork opacity-0 transition-opacity group-hover:opacity-100">
                  Läs mer →
                </span>
              </button>
            ))}
      ...
      {/* ── CTA (mässing-band istället för mörkt – varmt och ljust) ── */}
      <section className="relative overflow-hidden bg-ek-700">
        <div className="absolute inset-0 papperskorn opacity-20" />
        <div className="relative mx-auto max-w-content px-6 py-20 text-center">
          ...
          <p className="mx-auto mt-4 max-w-lg text-ek-100/90">
            Berätta om din idé, så ser vi tillsammans vad som är möjligt.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <button
              onClick={() => navigera('boka')}
              className="knapp-primar bg-ek-50 text-grafit-900 hover:bg-massing-ljus"
            >
              Boka ett möte
            </button>
            <button
